...
Este archivo es parte del Proyecto Auditoria 2026-1.
...
# Integrar capa estática
from static_serve import setup_static_routes
setup_static_routes(app)

# Endpoint para pre-deploy checks
@app.route('/pre-deploy-check', methods=['GET'])
def pre_deploy_check():
    """Endpoint para verificación pre-deploy"""
    return jsonify({
        "status": "ready",
        "checks": {
            "static_files": "read_only",
...
def health_detailed():
    ...
    return jsonify({
        ...
        "components": {
            ...
            "rate_limiting": f"{len(login_attempts)} active monitors"
        },
        "stats": {
            "users": len(users_db),
            "static_files": len([f for f in os.listdir(STATIC_DIR) if os.path.isfile(os.path.join(STATIC_DIR, f))]) if os.path.exists(STATIC_DIR) else 0
        }
    })
